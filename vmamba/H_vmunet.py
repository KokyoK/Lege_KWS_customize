...
class H_vmunet(nn.Module):
    ...
    def __init__(self, num_classes=1, input_channels=3,layer_scale_init_value=1e-6,H_SS2D=H_SS2D, block=Block,pretrained=None,
        ...
        self.encoder1 = nn.Sequential(
            nn.Conv2d(input_channels, c_list[0], kernel_size=(1, 3), stride=1, padding=1),
        )
        self.encoder2 =nn.Sequential(
            nn.Conv2d(c_list[0], c_list[1], kernel_size=(1, 3), stride=1, padding=(1,1)),
        ) 
        ...
        self.encoder3 = nn.Sequential(
            *[block(dim=c_list[1], drop_path=dp_rates[0 + j],
            ...
            nn.Conv2d(c_list[1], c_list[2], kernel_size=(1, 3), stride=1, padding=1),
        )
        ...
        self.encoder4 = nn.Sequential(
            *[block(dim=c_list[2], drop_path=dp_rates[2 + j],
            ...
            nn.Conv2d(c_list[2], c_list[3], kernel_size=(1, 3), stride=1, padding=1),
        )
        ...
        self.encoder5 = nn.Sequential(
            *[block(dim=c_list[3], drop_path=dp_rates[4 + j],
            ...
            nn.Conv2d(c_list[3], c_list[4], kernel_size=(1, 3), stride=1, padding=1),
            )
        ...
        self.encoder6 = nn.Sequential(
            *[block(dim=c_list[4], drop_path=dp_rates[6 + j],
            ...
            nn.Conv2d(c_list[4], c_list[5], kernel_size=(1, 3), stride=1, padding=1),
            )
        ...
        self.decoder1 = nn.Sequential(
            *[block(dim=c_list[5], drop_path=dp_rates[6],
            layer_scale_init_value=layer_scale_init_value, H_SS2D=H_SS2D[3]) for j in range(depths[3])],
            nn.Conv2d(c_list[5], c_list[4], kernel_size=(1, 3), stride=1, padding=1),
            )
        ...
        self.decoder2 = nn.Sequential(
            *[block(dim=c_list[4], drop_path=dp_rates[4+j],
            layer_scale_init_value=layer_scale_init_value, H_SS2D=H_SS2D[2]) for j in range(depths[2])],
            nn.Conv2d(c_list[4], c_list[3], kernel_size=(1, 3), stride=1, padding=1),
            )

        self.decoder3 = nn.Sequential(
            *[block(dim=c_list[3], drop_path=dp_rates[2+j],
            layer_scale_init_value=layer_scale_init_value, H_SS2D=H_SS2D[1]) for j in range(depths[1])],
            nn.Conv2d(c_list[3], c_list[2], kernel_size=(1, 3), stride=1, padding=1),
            )

        self.decoder4 = nn.Sequential(
            *[block(dim=c_list[2], drop_path=dp_rates[0+j],
            layer_scale_init_value=layer_scale_init_value, H_SS2D=H_SS2D[0]) for j in range(depths[0])],
            nn.Conv2d(c_list[2], c_list[1], kernel_size=(1, 3), stride=1, padding=1),
            )

        self.decoder5 = nn.Sequential(
            nn.Conv2d(c_list[1], c_list[0], kernel_size=(1, 3), stride=1, padding=1),
        )  
...
if __name__ == "__main__":
    ...
    device = "cuda"
    model = H_vmunet(num_classes=188, 
                input_channels=40, 
                c_list=model_cfg['c_list'], 
                split_att=model_cfg['split_att'], 
                bridge=model_cfg['bridge'],
                drop_path_rate=model_cfg['drop_path_rate']).to(device)
    input_tensor = torch.randn(10, 40, 1, 101).to(device)  # 示例批次大小为 10
