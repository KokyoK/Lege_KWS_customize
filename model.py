...
class SiameseTCResNet(nn.Module):
    ...
    def forward(self, input1, input2):
        # 分别处理两个输入
        out_k1, out_s1, map_k1, map_s1 = self.network(input1)
        out_k2, out_s2, map_k2, map_s2 = self.network(input2)

        # 计算相似度 - 这里以余弦相似度为例
        similarity = F.cosine_similarity(map_s1.view(map_s1.size(0), -1),
                                         map_s2.view(map_s2.size(0), -1))

        return out_k1, map_s1, out_k2, map_s2, similarity
